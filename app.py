...
# ==============================================================================
# 0. CONFIGURACIÓN VISUAL (ESTABILIDAD TOTAL - TEMA CLARO)
# ==============================================================================
st.set_page_config(
    page_title="HYDROLOGIC | Suite",
    page_icon="💧",
    layout="wide",
...
def local_css():
    ...
    <style>
        ...
        /* --- 1. FORZADO NUCLEAR DE TEMA CLARO (NO MÁS ERRORES) --- */
        ...
        /* --- 2. INPUTS (EL PROBLEMA PRINCIPAL SOLUCIONADO) --- */
        /* Forzamos que las cajas de texto sean blancas con letras negras */
        ...
        div[data-testid="stMetricValue"] { color: #0284c7 !important; font-weight: 800 !important; font-size: 1.8rem !important; }
        ...
        div.stButton > button:first-child p { color: #ffffff !important; } /* Texto blanco forzado */
        ...
        .tank-val { color: #0f172a; font-weight: 900; font-size: 2rem; }
...
def check_auth():
    ...
    with c2:
        ...
        st.markdown('<p class="brand-sub">ENGINEERING SUITE</p>', unsafe_allow_html=True)
...
silex_db = [
    ...
    Filtro("Silex", "SIL 12x48", "12x48", 1.1, 3.0), Filtro("Silex", "SIL 18x65", "18x65", 2.6, 6.0),
    Filtro("Silex", "SIL 21x60", "21x60", 3.6, 9.0), Filtro("Silex", "SIL 24x69", "24x69", 4.4, 12.0),
    Filtro("Silex", "SIL 30x72", "30x72", 7.0, 18.0), Filtro("Silex", "SIL 36x72", "36x72", 10.0, 25.0)
]
carbon_db = [
    Filtro("Carbon", "DEC 30L", "10x35", 0.38, 1.5), Filtro("Carbon", "DEC 45L", "10x54", 0.72, 2.0),
    Filtro("Carbon", "DEC 60L", "12x48", 0.80, 2.5), Filtro("Carbon", "DEC 75L", "13x54", 1.10, 3.5),
    Filtro("Carbon", "DEC 90KG", "18x65", 2.68, 6.0), Filtro("Carbon", "DEC 150KG", "21x60", 4.5, 9.0),
    Filtro("Carbon", "DEC 200KG", "24x69", 6.0, 12.0)
]
descal_db = [
    ...
    Filtro("Descal", "TWIN 140L", "16x65", 6.0, 6.0, 25.0, 896), Filtro("Descal", "DUPLEX 300L", "24x69", 6.5, 9.0, 45.0, 1800)
]
...
with col_sb:
    ...
    horas = st.number_input("Horas Prod", value=20, max_value=22)
...
if st.session_state.get('run'):
    ...
    if res.get('ro') or res.get('descal'):
        ...
        if res.get('v_buffer', 0) > 0:
            c1.markdown(f"<div class='tank-container tank-intermedio'><div class='tank-header'>🛡️ BUFFER</div><div class='tank-val'>{int(res['v_buffer'])} L</div></div>", unsafe_allow_html=True)
        with c2 if res.get('v_buffer', 0) > 0 else c1:
            c2.markdown(f"<div class='tank-container tank-final'><div class='tank-header'>🛢️ DEPÓSITO FINAL</div><div class='tank-val'>{int(res['v_final'])} L</div></div>", unsafe_allow_html=True)
