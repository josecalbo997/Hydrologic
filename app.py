...
# ==============================================================================
# 3. GENERADOR PDF (SOPORTE IMÁGENES FLEXIBLE)
# ==============================================================================
def create_pdf(res, inputs, modo, user_data):
    pdf = FPDF()
    pdf.add_page()
    
    # 1. LOGO DINÁMICO (PROTEGIDO)
    logo_impreso = False
    if user_data.get("logo_url") and len(str(user_data["logo_url"])) > 5:
        try:
            response = requests.get(user_data["logo_url"])
            if response.status_code == 200:
                # Detectar tipo de imagen por extensión o cabecera
                # FPDF prefiere PNG/JPG. Si es WebP u otro, FPDF puede fallar.
                # Aquí intentamos guardarlo con la extensión correcta.
                ext = ".png" # default
                if "jpeg" in response.headers.get("Content-Type", ""): ext = ".jpg"
                
                with tempfile.NamedTemporaryFile(delete=False, suffix=ext) as tmp:
                    tmp.write(response.content)
                    # Convertir a PNG si es necesario usando PIL (Pillow) para máxima compatibilidad
